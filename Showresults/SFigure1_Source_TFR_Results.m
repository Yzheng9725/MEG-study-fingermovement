%% This figure is shown in Supplementary Figure 1.
%%
clc; clear; close all;
ft_defaults;
...
%% Load data - Each task TFR data and TFCE data
load(fullfile(opt.groupmatdir, 'TFR_Group_AllType_AllTask.mat'));

%% Show results - All task average TFR and stat
taskName = {'Thumb', 'Index', 'Middle', 'Little'};
typeName = {'TFR_full', 'TFR_evoked', 'TFR_induced'};
climits = [-6 6]; % for t-value
for taskidx = 1:numel(taskName)
    for typeidx = 1:numel(typeName)
        tf_tmp = TFR_Group_AllType_AllTask_Stat_TFCE.(taskName{taskidx}).(typeName{typeidx});
        % Show results
        ...
        clim(climits);
        set(fig, 'Name', sprintf('Supplementary_Stat_TFCE_%s_%s', taskName{taskidx}, typeName{typeidx}));
        drawnow;
