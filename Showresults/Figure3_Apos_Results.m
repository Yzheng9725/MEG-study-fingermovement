%% This figure is shown in Figure 3 and Supplementary Figure 2.
%% 
clc; clear; close all;
ft_defaults;
...
%% Plot max/min AI pos (Using all signals)
% Load AI results - max AI pos - Actseg
load(fullfile(opt.groupmatdir, 'showresults_maxAI_value_pos_Actseg.mat'));
...
for f = 2:7 % Delta/Theta/Alpha/Beta/Gamma/High-gamma
    ...
    for t = 1:4 % Thumb/Index/Middle/Little
        % draw pos maxidx
        pos2plot = surf_real.pos(squeeze(maxidx{f}(:,t)),:);
        scatter(pos2plot(:,1), pos2plot(:,2), psize(1), repmat(cm(t,:), size(pos2plot, 1), 1), 'o', 'filled', 'MarkerFaceAlpha', 0.3);
        % draw pos minidx
        pos2plot = surf_real.pos(squeeze(minidx{f}(:,t)),:);
        scatter(pos2plot(:,1), pos2plot(:,2), psize(1), repmat(cm(t,:), size(pos2plot, 1), 1), 'square', 'filled', 'MarkerFaceAlpha', 0.3);
    end
    ...
    for t = 1:4 % Thumb/Index/Middle/Little
        % draw pos maxidx
        pos2plot = surf_real.pos(squeeze(maxidx{f}(:,t)),:);
        pos_avg2plot = mean(pos2plot, 1);
        scatter(pos_avg2plot(1), pos_avg2plot(2), psize(2), cm(t,:), 'o', 'filled', 'MarkerFaceAlpha', 1);
        % draw pos minidx
        pos2plot = surf_real.pos(squeeze(minidx{f}(:,t)),:);
        pos_avg2plot = mean(pos2plot, 1);
        scatter(pos_avg2plot(1), pos_avg2plot(2), psize(2), cm(t,:), 'square', 'filled', 'MarkerFaceAlpha', 1);
    end
    set(fig, 'Name', sprintf('maxminAI_Pos_%s_Actseg_Lcortex_flat', FN{f}));
    drawnow;
    print(fig, fullfile(opt.groupresultdir, strcat(fig.Name, '.png')), '-dpng', '-r600', '-image');
    % print(fig, fullfile(opt.groupresultdir, strcat(fig.Name, '.eps')), '-depsc2', '-r600', '-vector');
end
...
for t = 1:4
    ...
    colorbar('Location', 'southoutside', 'Ticks', [], 'box', 'on');
    xlim([1 60]); ylim([0 1]);
    axis off;
    if t == 1
        title('Thumb');
