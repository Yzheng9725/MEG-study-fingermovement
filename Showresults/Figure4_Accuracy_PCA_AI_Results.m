...
%%
load(fullfile(opt.groupmatdir, 'meanACC_pca_70var_ACTseg_-1_2s_group_16sub.mat'));
load(fullfile(opt.groupmatdir, 'meanACC_maxAI_ACTseg_-1_2s_group_16sub.mat'));
load(fullfile(opt.groupmatdir, 'meanACC_maxAI_winappend_-1_2s_group_16sub.mat'));

Acc_show(:,:,1) = meanACC_pca_group(:,2:7);
Acc_show(:,:,2) = meanACC_maxAI_group(:,2:7);
Acc_show(:,:,3) = meanACC_maxAI_winappend_group(:,2:7);

%% Plot using scatter points
subname = FN(2:7);
acc2show = Acc_show;
accmean = [squeeze(mean(Acc_show(:,:,1)*100))' squeeze(mean(Acc_show(:,:,2)*100))' squeeze(mean(Acc_show(:,:,3)*100))'];
accstd = [std(Acc_show(:,:,1)*100)' std(Acc_show(:,:,2)*100)' std(Acc_show(:,:,3)*100)'];
...
for h = 1:numel(hbar)
    ...
    for k = 1:length(hbar(h).XEndPoints)
        hs(h) = scatter(repelem(hbar(h).XEndPoints(k), size(acc2show, 1)), acc2show(:,k,h)*100, 15, k*ones(size(acc2show, 1), 1),...
...
colormap(colors);
ylim([0 80]); set(gca, 'YTick', 0:10:80); ylabel('Accuracy (%)');
xlim([0.5 6.5]); set(gca, 'XTick', 1:6, 'XTickLabel', subname);
set(gca, 'Color', 'none');
legend(hs, {'PCA', 'A-epoch', 'A-window'}, 'Box', 'off', 'Location', 'northeast');
set(fig, 'Name', 'showresults_Acc_PCA_MaxAI_winappend_compar_barscatter_symbol');
...
for i = 1:size(accavg, 1)
    % herrbar(i) = errorbar((1:size(accavg,2)), accavg(i,:), accstd(i,:), 'LineStyle', '-', 'LineWidth', 1.2, 'Color', colors(i,:), 'Marker', '^', 'MarkerSize', 5, 'MarkerFaceColor', colors(i,:), 'MarkerFaceAlpha', 0.6, 'MarkerEdgeColor', 'none'); % 
...
ylim([0 80]); set(gca, 'YTick', 0:10:80); ylabel('Accuracy (%)');
xlim([0.5 3.5]); set(gca, 'XTick', 1:3, 'XTickLabel', {'PCA', 'A-epoch', 'A-window'});
set(gca, 'Color', 'none');
set(fig, 'Name', 'showresults_Acc_PCA_MaxAI_winappend_compar_method');
print(fig, fullfile(opt.groupresultdir, strcat(fig.Name, '.png')), '-dpng', '-r600', '-image');
% print(fig, fullfile(opt.groupresultdir, strcat(fig.Name, '.eps')), '-depsc2', '-r600', '-vector');
