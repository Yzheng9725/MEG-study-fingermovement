function varargout = wxyz_colormap_gradient(varargin)

%%

wxyzColor_gradient = wxyz_color_gradient;
wxyzColormap_gradient = cell(size(wxyzColor_gradient));

%% Generate
N = 256;
for c = 1:length(wxyzColor_gradient)
    wxyzColormap_gradient{c} = GenColormap(wxyzColor_gradient{c}, N);
end

%% 
code = 0;
if nargin == 1
    if isnumeric(varargin{1}) && varargin{1} >= 1 && varargin{1} <= length(wxyzColor_gradient)
        code = varargin{1};
    else
        error(['The intput varaiable should be larger than 1 and less than ' num2str(length(wxyzColor_gradient))]);
    end
else
...
%% Output
varargout{1} = wxyzColormap_gradient{code};
...
% for c = 1:length(wxyzColor_gradient)
%     nexttile;
%     image(permute(wxyzColormap_gradient{c}, [3,1,2])); axis off;
%     title(['No.' num2str(c)], 'FontName', 'Times New Roman', 'FontSize', 10, 'FontWeight', 'bold');
% end
% print(h, 'Example_gradient', '-dpng', '-r300', '-image')
